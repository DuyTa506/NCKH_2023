...
parser.add_argument('--model', default='VGG16', help='VGG16 | IR152 | FaceNet64 | DP_SGD')
...
parser.add_argument('--bs', type= int, default= 350, help='batch size for top n selection')
args = parser.parse_args()


class PublicFFHQ(torch.utils.data.Dataset):
    def __init__(self, root='E:/NCKH2023/LPG-MI/datasets/thumbnails128x128', transform=None):
...
class PublicCeleba(torch.utils.data.Dataset):
    def __init__(self, file_path='E:/NCKH2023/LPG-MI/data_files/celeba_ganset.txt',
                 img_root='E:/NCKH2023/LPG-MI/datasets/img_align_celeba', transform=None):
...
if model_name_T.startswith('DP_SGD') :
    ...
    path_T = './checkpoints/target_model/target_ckp/VGG_MixedGhost.tar'
elif model_name_T.startswith('IR152'):
    T = IR152(1000)
    path_T = './checkpoints/target_model/IR152_91.16.tar'
elif model_name_T == "FaceNet64":
    T = FaceNet64(1000)
    path_T = './checkpoints/target_modelW/aceNet64_88.50.tar'
...
if args.data_name == 'celeba':
    ...
    data_set = PublicFFHQ(root='./datasets/thumbnails128x128/', transform=ffhq_transform)
    data_loader = data.DataLoader(data_set, batch_size=args.bs)
